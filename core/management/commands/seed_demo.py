...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        category_data = [
            ("العناية بالبشرة", "منتجات تمنح البشرة الترطيب والإشراقة.", "serum.webp"),
            ...
            ("المكياج", "أساسيات خفيفة لإطلالة طبيعية.", "cream.webp"),
        ...
        product_data = [
            ...
            ("كريم الترطيب المخملي", "LUM-SKN-002", "العناية بالبشرة", "cream.webp", "راحة يومية وحاجز ترطيب متوازن.", "كريم غني سريع الامتصاص للعناية بالبشرة الجافة والعادية.", "سيراميد، سكوالان، زبدة الشيا", "يُستخدم صباحًا ومساءً.", "285", None, True, False),
            ("بودي ميست نسمة الورد", "LUM-BDY-001", "العناية بالجسم", "mist.webp", "رائحة هادئة تدوم بخفة طوال اليوم.", "رذاذ عطري ناعم بنفحات ورد دافئة ولمسة فانيليا.", "ماء عطري، جلسرين نباتي", "يُرش على الجسم من مسافة 15 سم.", "219", "260", True, True),
            ("زيت الجسم الكهرماني", "LUM-BDY-002", "العناية بالجسم", "serum.webp", "نعومة ولمعان من دون ملمس دهني.", "مزيج زيوت خفيف يدعم مرونة البشرة ويتركها ناعمة.", "زيت اللوز، الجوجوبا، فيتامين هـ", "تُدلّك كمية قليلة بعد الاستحمام.", "310", None, False, True),
        ...
        hero, _ = Banner.objects.get_or_create(position=Banner.Position.HERO, order=1, defaults={"title": "جمال طبيعي يبدأ بعناية صحيحة"})
        hero.title = "جمال طبيعي يبدأ بعناية صحيحة"
        hero.subtitle = "منتجات مختارة بعناية لروتين يومي أبسط ونتائج ملموسة."
        hero.button_text = "عرض المجموعة"
        ...
        promo.subtitle = "روتين بسيط بتركيبات مختارة يمنح البشرة عناية متوازنة كل يوم."
        promo.button_text = "عرض التفاصيل"
        ...
        page_data = {
            ...
            "سياسة-الخصوصية": ("سياسة الخصوصية", "نستخدم البيانات فقط لتنفيذ الطلبات وتحسين تجربة المتجر، ولا نبيع بيانات العملاء لأي جهة. نحافظ على البيانات وفق الممارسات الأمنية المناسبة."),
            "الشروط-والأحكام": ("الشروط والأحكام", "يعني استخدام المتجر الموافقة على تقديم بيانات صحيحة عند الطلب، وعلى سياسات الشحن والاستبدال الموضحة. الأسعار والمخزون يخضعان للتحديث."),
        }
        ...
        for index, (title, description, category_name, product_index) in enumerate([
            ...
            ("السيروم", "تركيز يناسب احتياجات البشرة", "العناية بالبشرة", 0),
            ("الترطيب", "احبسي الترطيب طوال اليوم", "العناية بالبشرة", 1),
            ("عناية الجسم", "خطوة ناعمة تكمل الروتين", "العناية بالجسم", 2),
        ], 1):
